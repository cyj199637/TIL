...
(참고: https://www.geeksforgeeks.org/cpu-scheduling-in-operating-systems/)

![Untitled](./image/img_34.png)

### FCFS(first-come, first-served)

- 스케줄링 알고리즘 중에서 가장 간단하다.
- CPU를 요청한 순서대로 CPU를 할당받는다.
- FIFO queue를 사용하여 구현된다.
- 비선점 방식

**장점:**

- 구현하고 사용하기 쉽다.
- 각 프로세스의 작업 시간이 긴 배치 시스템에 적합

**단점:**

- 평균 대기 시간이 다른 알고리즘보다 훨씬 높다.
    - 큐의 끝에 있는 프로세스는 완료까지 더 오래 기다려야 한다.
- Convoy effect 발생
    - `Convoy effect`

      : CPU를 매우 오래 사용하는 프로세스가 도착하게 되면, 다른 프로세스가 CPU를 사용하는 데 기다리는 대기 시간이 매우 커지는 현상

- 하나의 프로세스의 작업이 끝날 때까지 다음 프로세스를 실행할 수 없기 때문에 멀티태스킹 환경에서는 적합하지 않다.
...
- 프로세스의 다음 CPU burst가 가장 짧은 프로세스부터 실행
    - 늦게 도착하더라도 CPU burst가 앞에 대기중인 프로세스보다 짧으면 먼저 CPU를 할당받을 수 있다.
...
- 평균 대기 시간이 짧다.
- 일반적으로 장기 스케줄링에 사용된다.
- 실행 시간이 예측 가능한 배치 작업에 적합

**단점:**

- 계속해서 CPU burst가 짧은 프로세스가 계속 들어오면 CPU burst가 긴 프로세스에게 기아 문제가 발생할 수 있다.

  → 짧은 프로세스가 계속 추가되면 오랜 프로세스가 무기한 대기 상태에 놓일 수 있다.

    - 노화 기법으로 해결 가능
        - `노화 기법`

          : 오랫동안 기다린 프로세스에게 기다린 시간에 비례하여 우선순위를 높여주는 기법

- 다음으로 실행할 CPU 요청의 길이를 예측하기가 어렵다.
    - 따라서, 실행 시간을 정확하게 예측할 수 있는 특수 환경에서 사용할 수 있다.
...
- CPU burst가 짧은 프로세스가 매우 빠르게 처리됩니다.
- 시스템에서는 프로세스가 완료되거나 새 프로세스가 추가될 때만 결정하므로 다음 실행할 프로세스를 결정하는데에 오버헤드가 매우 적다.
    - 새 프로세스가 추가되면 알고리즘은 현재 실행 중인 프로세스를 새 프로세스와 비교하기만 하면 되고 현재 실행 대기 중인 다른 모든 프로세스는 무시하기 때문

**단점:**

- SJF처럼 프로세스 기아 문제가 발생할 수 있다.
...
- 우선 순위가 높은 프로세스부터 실행
    - 우선 순위가 같은 프로세스가 여러 개인 경우, FCFS 알고리즘을 기반으로 동작
- 선점 방식
    - 낮은 우선 순위의 작업이 실행되는 동안 높은 우선 순위의 작업이 도착하면, 낮은 우선 순위의 작업을 중단하고 높은 우선 순위의 작업을 실행한다.
...
- 우선 순위가 낮은 프로세스에게 기아 문제가 발생할 수도 있다.

<br/>

### RR(round-robin)

- 타임 슬라이스로 나눠진 CPU time을 가지고 번갈아가면서 실행
- FCFS 알고리즘의 선점형 버전

  → 프로세스가 CPU를 할당받은 시간만큼만 사용하고 물러나야하기 때문

- 멀티태스킹 방식과 가장 유사한 방식

**장점:**

- 모든 프로세스가 동일한 time slice를 할당 받으므로 간단하고 사용하기 쉽고 기아 현상이 없다.
...
- 프로세스들을 그룹화해서 그룹마다 큐를 두는 방식
    ...
    - 보통 foreground(interactive) 프로세스와 background(batch) 프로세스로 분류하며, foreground 프로세스 그룹의 큐가 우선 순위가 높다.

![untitled](./image/img_35.png)

- `시스템 프로세스`: CPU 자체에서 실행하는 프로세스
- `인터랙티브 프로세스`: 사용자와 직접적인 상호 작용하는 프로세스

**장점:**

- 프로세스는 대기열에 영구적으로 할당되므로 일정 오버헤드가 적다.

**단점:**

- 더 높은 우선순위 대기열이 절대 비워지지 않는 경우 일부 프로세스는 기아 문제가 발생할 수 있다.
